...
MandelTools : MandelModule {
	...
	*new {|hub|
	...
	metro {|pan=0.0, quant=4|
		var server = hub.server ? Server.default;
		this.stopMetro;
		...
		server.waitForBoot({
			...
			metro = Pbind(\server, server, \instrument, \mhTestClick, \dur, 1, \octave, 6, \pan, pan, \degree, Pseq([7,Pn(0,quant-1)],inf)).play(hub.clock, quant:quant);
		});
	}
	...
	impulseMetro {
		...
		server.waitForBoot({
			...
			metro = Pbind(\server, server, \instrument, \mhTestImpulse, \dur, 1, \amp, 1).play(hub.clock);
		});
	}
